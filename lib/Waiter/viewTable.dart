...
class _ViewTableState extends State<ViewTable> {
                    ...
                    content: SingleChildScrollView(
                      child: ListBody(
                        children: const <Widget>[
                          Text(
                              'Your Waiter account has been deactivated by your manager.'),
                        ],
                      ),
                    ),
                    actions: <Widget>[
  ...
  Future joinTable() async {
    ...
    var table = await FirebaseFirestore.instance
        .collection('tables')
        .doc(widget.tableID)
        .get();
    ...
    await FirebaseFirestore.instance
        ...
        .where('waiterID', isEqualTo: 'unhandled')
        .get()
        .then((orders) {
      if (orders.size != 0) {
  ...
  Future<void> clearTable(String userID, String tableID) async {
    ...
    CollectionReference pastVisitCollRef =
        FirebaseFirestore.instance.collection('users/$userID/pastVisits');

    String visitID = pastVisitCollRef.doc().id.toString().trim();
    ...
    FirebaseFirestore.instance
        .collection('tables/$tableID/tableOrders')
        .get()
        .then((value) {
      for (int i = 0; i < value.size; i++) {
        if (value.docs[i].data()['itemName'] != 'Request Waiter' &&
          ...
          FirebaseFirestore.instance
              .collection('users/$userID/pastVisits/$visitID/tableOrders')
              .doc()
              .set({
